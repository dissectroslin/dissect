...
GWAS::GWAS()
{
  ...
  this->genotypeFilesType = GenotypeAttributes::calls;
  ...
  else if(options.genotypeAndGRMsListFile != "" || options.genotypeBGENAndGRMsListFile != "" )
  {
    std::string usedFile;
    std::map<std::string, std::vector<std::string> > grms2genotypes;
    if( options.genotypeAndGRMsListFile != "" )
    {
      usedFile = options.genotypeAndGRMsListFile;
    ...
    {
      usedFile = options.genotypeBGENAndGRMsListFile;
      this->genotypeFilesType = GenotypeAttributes::probabilities;
      std::vector< std::vector<std::string> > files;
      getTableFromFile(options.genotypeBGENAndGRMsListFile, files, 2);
      for(int i = 0; i<files.size(); i++) //We load genotype and grm files in this way, because we want all genotype with same grms together in the list.
      {
        misc.checkFileExists(files[i][0]);
        misc.checkFileExists(files[i][1] + ".grm.ids");
        misc.checkFileExists(files[i][1] + ".grm.dat");
        misc.checkFileExists(files[i][1] + ".grm.snps");
        grms2genotypes[ files[i][1] ].push_back( files[i][0] );
  ...
  else if( options.genotypeAndResidualsListFile != "" || options.genotypeBGENAndResidualsListFile != "" )
  {
    loadGenotypeResidualFiles();
  }
  else
  {
...
void GWAS::computeGWAS()
{
  ...
  {
    misc.error("Error: An internal error has happened. Standard GWAS can not use BGEN files, yet.", 0);
  }
  ...
  for(int i = 0; i<this->genotypeFiles.size(); i++)
  {
    ...
    std::vector<std::string> prefilterCovInds = intersectionStringVectors(2, &genotype->individualIds, &phenotype->individualIds);
    std::set<std::string> setPrefilterCovInds(prefilterCovInds.begin(), prefilterCovInds.end());
    Covariate * covariate = new Covariate(options.covarsFile, options.qCovarsFile, phenotype->individualIds, true, setPrefilterCovInds);
    ...
    else
    {
      ...
      bool success = computeCovariance(genotype, phenotype, covariate);
      if(success == false)
      {
    ...
    if(options.analysis != recursiveGWASAnalysis) //Perform grouped GWAS or standard GWAS
    {
      ...
      else
      {
        ...
        {
          computeIndividualGWASGroupedCommunicator(genotype, phenotype, covariate);
        }
      }
...
void GWAS::computeGroupedGWAS(Genotype * genotype, Phenotype * phenotype, Covariate * covariate)
{
  ...
  LabeledMatrix * groupEffects = NULL;
  if( options.saveGroupEffects == true )
  {
    groupEffects = new LabeledMatrix(genotype->genotypes->nGlobCols, nGroups);
  }
  
  std::vector<std::string> groupsDone;
  std::vector<std::string> groupsDoneWithSuccess;
  
  int stepsForPercentageOutput = (nGroups/10);
  ...
  for(int i = 0; i < nGroups; i++)
  {
    ...
    if(communicator->mpiRoot)
    {
      ...
      groupsDone.push_back(group);
      
      if( i % stepsForPercentageOutput == 1 )
      {
        misc.message << (100*i)/nGroups << "% completed." << std::endl;
      }
    }
    ...
    while(iterate == true)
    {
      ...
      if(success == true)
      {
        ...
        {
          computeGroupVariance(groupResults, gTranspose, groupEffects, i);
          if(communicator->mpiRoot)
          {
            results[group] = groupResults;
            resultSNPInfo[group] = groupGenotype->SNPs;
            
            groupsDoneWithSuccess.push_back( group );
          }
      ...
      //If fitting failed, try to filter dependent SNPs in case they are not already filtered.
      if(success == false)
      {
  ...
  if(groupEffects != NULL)
  {
    if( options.saveGroupEffects == true )
    {
      groupEffects->setColLabels(groupsDone);
      groupEffects->setRowLabels(genotype->individualIds);
      
      groupEffects->filterRowsAndCols(genotype->individualIds, groupsDoneWithSuccess);
      
      groupEffects->save(options.outFile + ".effects");
    }
    delete groupEffects;
    groupEffects = NULL;
  }
...
void GWAS::computeIndividualGWAS(Genotype * genotype, Phenotype * phenotype, Covariate * covariate)
{
  ...
  int stepsForPercentageOutput = (genotype->nSNPs/10);
  stepsForPercentageOutput = (stepsForPercentageOutput == 0?1:stepsForPercentageOutput);
  
  for(int isnp = 0; isnp < genotype->nSNPs; isnp++ )
  {
    if( isnp % stepsForPercentageOutput == 1 )
    {
      misc.message << (100*isnp)/genotype->nSNPs << "% completed." << std::endl;
    }
    ...
    {
      bool succesGroupSignificance = computeGroupSignificance(phenotype->phenotypes, reducedResults, SNPResults);
...
void GWAS::computeIndividualGWASGroupedCommunicator(Genotype * gcgenotype, Phenotype * gcphenotype, Covariate * gccovariate)
{
  ...
  for(int isnp = 0; isnp < genotype->nGlobRows; isnp++ )
  {
    ...
    Matrix * incidence = new Matrix(MATRIX_DEFAULT_DISTRIBUTION);
    incidence->joinMatricesHorizontally(covariate, snpData);
    
    GLMResults SNPResults;
    Matrix * fixedEffects = new Matrix(MATRIX_DEFAULT_DISTRIBUTION);
    bool success = computeGLM(phenotype, incidence, fixedEffects, SNPResults);
    ...
    if(communicator->mpiRoot)
    {
      bool succesGroupSignificance = computeGroupSignificance(phenotype, reducedResults, SNPResults);
...
void GWAS::computeGroupVariance(GLMResults & results, Matrix * genotypes, LabeledMatrix * groupEffects, int idx)
{
...
void GWAS::gatherResults(std::map<std::string, GLMResults > & results, std::map<int, GLMResults > & groupedResults, std::map<std::string, std::vector<SNP> > & resultSNPInfo, Genotype * genotype, std::vector<std::string> & unfittedSNPs)
{
  results.clear();
  resultSNPInfo.clear();
...
bool GWAS::computeCovariance(Genotype * genotype, Phenotype * phenotype, Covariate * covariate)
{
